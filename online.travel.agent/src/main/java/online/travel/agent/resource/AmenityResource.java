...
public class AmenityResource {
	...
	public Response addAmenity(Amenity amenity) {
		...
		try {
			...
			LOG.warn(e);
			response = Response.serverError().entity("Exception in adding amenity: " + e.getMessage()).build();
		}
		return response;
	}
	...
	public Response getAmenity(@PathParam("id") int id) {
		...
		try {
			...
			if (result != null) {
				...
				response = Response.serverError().entity("Amenity with given id is not found").build();
			}
		} catch (Exception e) {
			LOG.warn(e);
			response = Response.serverError()
					.entity("Exception in fetching the amenity with given id: " + e.getMessage()).build();
		}
		return response;
	}
	...
	public Response deleteAmenity(@PathParam("id") int id) {
		...
		try {
			...
			LOG.warn(e);
			response = Response.serverError()
					.entity("Exception in deleting the amenity with given id: " + e.getMessage()).build();
		}
		return response;
	}
