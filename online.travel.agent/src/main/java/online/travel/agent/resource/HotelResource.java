...
public class HotelResource {
	...
	try {
		...
		response = Response.noContent().entity("Couldn't find/delete the amenity").build();
	} catch (Exception ex) {
	    LOG.warn(ex);
	    response = Response.noContent()
	...
	try {
	    ...
	    LOG.warn(ex);
	    response = Response.noContent().entity("Exception in updating hotel: " + ex.getMessage()).build();
	}
	return response;
    }
}
