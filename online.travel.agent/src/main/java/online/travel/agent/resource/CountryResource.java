...
public class CountryResource {
	...
	public Response addCountry(Country country) {
		...
		try {
			...
			LOG.warn(e);
			response = Response.serverError().entity("Exception in adding country: " + e.getMessage()).build();
		}
		return response;
	}
	...
	public Response getCountry(@PathParam("id") int id) {
		...
		try {
			...
			if (result != null) {
				...
				response = Response.serverError().entity("Country with given id is not found").build();
			}
		} catch (Exception e) {
			LOG.warn(e);
			response = Response.serverError()
					.entity("Exception in fetching the country with given id: " + e.getMessage()).build();
		}
		return response;
	}
	...
	public Response deleteCountry(@PathParam("id") int id) {
		...
		try {
			...
			LOG.warn(e);
			response = Response.serverError()
					.entity("Exception in deleting the country with given id: " + e.getMessage()).build();
		}
		return response;
	}
